...
data_train <- ts_data |> 
  ...
  select(id, year, production, total_water, 
...
fm <- total_water ~ production + mine_type_combined + process_route + by_prod_group
md <- lm(fm, data = data_train)
summary(md)
...
fm <- total_water ~ production + mine_type_combined + process_route + ore_body_group + by_prod_group
md <- lm(fm, data = data_train)
summary(md)
