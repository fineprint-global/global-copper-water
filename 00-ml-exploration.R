...
data_prediction <- ts_data |> 
  select(id, year, total_water, production, ore_grade, by_prod_group, process_route, mine_type_combined) |> 
  filter(!str_detect(process_route, "other|pyro/hydro")) |> 
  filter(!str_detect(by_prod_group, "CuCu|CuNi|CuPt|CuCo")) |> 
...
fm <- total_water ~ production + ore_grade + by_prod_group + process_route + mine_type_combined
...
data_prediction$total_water_pred <- predict(model_rf, data_prediction)

select(sf_data, id) |> 
  left_join(data_prediction) |> 
  ggplot(aes(x = total_water_pred, y = total_water, label = id)) + 
  geom_point() + 
...
plot_goode_homolosine_world_map(ocean_color = "#e5f1f8", land_color = "gray95", 
  ...
  ggplot2::geom_sf(data = grid_cells, mapping = aes(fill = total_water_pred * 1e-6), color = NA, lwd = 0, size = 0) + # * 100 from km2 to ha
